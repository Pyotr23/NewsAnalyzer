...
export async function getNews<T>(question: string): Promise<T> {
...
export async function getInTitleNews<T>(question: string): Promise<T> {
  const dateTime = new Date();
  dateTime.setDate(dateTime.getDate() - DAYS_AGO_COUNT);
  const response =
    await fetch(`https://nomoreparties.co/news/v2/everything?qInTitle=${question}&apiKey=${API_KEY}&pageSize=${PAGE_SIZE}&from=${dateTime.toISOString()}&sortBy=publishedAt`);
  return response.json() as Promise<T>;
}
